...
let gApi;
let gErr;
function incidencia(sgi, jquery, materialize, api, err, message) {
    tradProyectos = {};
    isSGI = sgi;
    $ = jquery;
    M = materialize;
    ...
    if (!isSGI) {
        api
            ...
            .then((data) => {
            if (!data || data.error === 'Insufficient permission')
                return;
            $('#sidenav .create-incidencia').removeClass('hide');
            $('.context-menu').addClass('context-menu-big');
            $('.context-menu .menu-options').appendChild($('<li class="menu-option create-incidencia">Reportar a DPS</li>'));
    ...
    $('body').appendChild($('<dps-spinner id="loader-incidencia" full-page></dps-spinner>'));
    $('body').appendChild($(`
  ...
  `;
    ...
    $('.help-tipo-incidencia').data('tooltip', `
    <ul>
    ...
    if (isSGI) {
        api
            ...
            proyectos.forEach((p) => {
                ...
                datalist.appendChild($(`<option value="${p.nombre}">${p.nombre}</option>`));
            });
            $('#proyecto-incidencia-crear-modal').prop('pattern', proyectos
...
function submitIncidencia(e) {
    e.preventDefault();
    const data = getDataExtraIncidencia();
...
function grabarPantalla() {
    ...
    M.Modal.getInstance($('#modal-incidencia-crear').el()).close();
    const win = window.open(url, 'reporting', 'status=no,width=1260,height=900');
    win.addEventListener('load', () => {
        setTimeout(() => {
            const descripcion = win.document.getElementById('descripcion-incidencia-crear-modal');
            const tipo = win.document.getElementById('tipo-incidencia-crear-modal');
            const descripcionLabel = win.document.getElementById('descripcion-incidencia-crear-label');
            const proyectoLabel = win.document.getElementById('proyecto-incidencia-crear-label');
            ...
            if (isSGI) {
                const proyecto = win.document.getElementById('proyecto-incidencia-crear-modal');
...
function getDataExtraIncidencia() {
    return {
        ...
        historial: JSON.parse((localStorage === null || localStorage === void 0 ? void 0 : localStorage.history) || '[]').slice(-10),
        url: { href: window.location.href },
        inputs: Object.fromEntries($('textarea , input:not([type="checkbox"]) , select , dps-select , dps-textarea').map((e) => [e.id || e.name, e.value])),
    };
}
